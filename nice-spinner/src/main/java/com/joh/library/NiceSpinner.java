...
public class NiceSpinner extends AppCompatTextView {
    ...
    //默认显示几行下拉
    private int dropDownShowCount = 5;
    private String hintText; // 添加提示文本变量
    private int hintTextColor = Color.GRAY; // 提示文本颜色，默认为灰色
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        setPadding(resources.getDimensionPixelSize(R.dimen.one_and_a_half_grid_unit), defaultPadding, defaultPadding,
                defaultPadding);
        setClickable(true);
        hintText = typedArray.getString(R.styleable.NiceSpinner_hintText); // 获取提示文本
        backgroundSelector = typedArray.getResourceId(R.styleable.NiceSpinner_backgroundSelector, R.drawable.selector);
        backgroundPop = typedArray.getResourceId(R.styleable.NiceSpinner_popBackgroundSelector, R.drawable.selector);

        setBackgroundResource(backgroundSelector);
        textColor = typedArray.getColor(R.styleable.NiceSpinner_textTint, getDefaultTextColor(context));
        hintTextColor = typedArray.getColor(R.styleable.NiceSpinner_hintTextColor, hintTextColor); // 获取提示文本颜色
        setTextColor(textColor);
        popupWindow = new ListPopupWindow(context);

        popupWindow.setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));

        //popupWindow.setBackgroundDrawable(ContextCompat.getDrawable(context, backgroundPop));
        ...
        popTextGravity = typedArray.getInt(R.styleable.NiceSpinner_popTextGravity, Gravity.CENTER);
        ...
        typedArray.recycle();
        if (hintText != null) {
            setText(hintText); // 初始显示提示文本
            setTextColor(hintTextColor); // 设置提示文本颜色
        }
    ...
    private void setTextInternal(Object item) {
        if (item != null) {
            ...
            setTextColor(textColor); // 选择项目时，使用默认文本颜色
        } else if (hintText != null) {
            setText(hintText);
            setTextColor(hintTextColor); // 显示提示文本时，使用提示文本颜色
        } else {
            setText("");
        }
    }
    ...
    public void setHintText(String hintText) {
        ...
        if (adapter == null || adapter.getCount() == 0) {
            setText(hintText); // 如果没有选择项，则显示提示文本
        }
    }
    /**
    ...
    public <T> void attachDataSource(@NonNull List<T> list) {
        adapter = new NiceSpinnerAdapter<>(getContext(), list, textColor, popTextGravity);
    ...
    public int getPopTextGravity() {
        return popTextGravity;
    }
    ...
    public void showDropDown() {
        ...
        if (listView != null) {
            ...
            listView.setBackground(ContextCompat.getDrawable(getContext(), backgroundPop));

            if (adapter != null && adapter.getCount() > 0) {
                if (adapter.getCount() > dropDownShowCount) {
                    int h = getHeight();
                    int w = getWidth();
                    popupWindow.setWidth(w);
                    popupWindow.setHeight(h * dropDownShowCount + listView.getDividerHeight() * (dropDownShowCount - 1));
                    requestLayout();
                    invalidate();
                }
            }
...
//        if (adapter == null || adapter.getCount() == 0) {
//            setTextColor(hintTextColor); // 如果没有选择项，则更新提示文本颜色
//        }
//    }
